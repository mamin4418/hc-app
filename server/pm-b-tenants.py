...
import utils.pm_mail, utils.pm_conf, ref_data, pm_update, pm_company
...
def noticeEmail(app, core):
    ...
    if ( utils.pm_mail.verifyEmailAddress(app, core['receiver_email']) < 0 ):
        app.logger.debug(appname+".noticeEmail.2"+core['receiver_email'])
        return 0

    core['receiver_name']=core['first_name']+" "+core['last_name']
    core['message_format']='html'
    if ( 'subject' not in core ):
        core['subject']='Himalaya Properties: Late Fees Re-instituted'
    ...
    if ( 'attachment' in core and core['attachment'] != ''):        
        ...
        utils.pm_mail.sendMail(app, core)
...
def paymentUpdates(app, db1, qj, tdate):
    ...
    c = O_t.getSTATS()
    app.logger.debug("paymentUpdates.5:", c)
       
    if ( c.empty ):
        app.logger.debug("paymentUpdates: NO PAYMENT FOR THIS TENANT:", data['tenant_id'], data['first_name'], data['last_name'])
        return
    h1 = json.loads(c.to_json(orient='table', double_precision=2))['data']
    #app.logger.debug(" format: length:",len(h1))
    i=0
    while i < len(h1):
...
def generateStatements(app, db1, qj, tdate):
    ...
    if ( 'email' not in data or data['email'] == None or  data['email'] == '' ):
        app.logger.error("generateStatements: EMAIL NOT AVAILABLE[%s]", qj['tenant_id'])
        return
    ...
    if ( not c.empty ):
        ...
        app.logger.debug("NO transaction to report, no statements will be generated and no email will eb sent")
        return
    ...
    if ( 'action' in qj and qj['action'] == 'STATEMENTS2'):
        ...
        if ( float(data['amount_due'].replace(' [Credit]','')) > 100 ):
            app.logger.debug("Amount due: %s < 100, no Email message, will be sent.",data['amount_due'])
    else:
    ...
    #c_pd = ref_data.get_company_info_pd(db1,app.logger)
    cp={}
    cp['id_type'] = "property_id"
    cp['id_value'] = data["property_id"]
    company = pm_company.getCompany(db1,app.logger, cp)
    #app.logger.debug("Company info:%s", company)
    data['company']=company['name']
    data['company_address']=company['m_address']
    data['company_address2']=company['m_city']+"  "+company['m_state']+" "+company['m_zip']
...
def processTenants(app, db1,action, tdate, company, attachment, group, tenants=""):
    
    qry =" SELECT tc.tenancy_id, t.tenant_id,t.first_name,t.last_name,ifnull(t.email,'') as email, t.phone, p.property_id, p.label, p.city, p.state, p.zip, "
    ...
    qry += " and p.llc=pc.label and t.status=1 and tc.status in (1,4,5) "
    ...
    #qry += " and t.tenant_id > 399 "
    qry += " order by t.tenant_id "
    i=0
    app.logger.debug("processTenants.1:%s", qry)    
    tc_list = db1.query_list(qry, None)
    
    #print(tc_list)
    for x in tc_list:
        app.logger.debug("[%s]TENANT = [%s]",i,x)
        i += 1
        if ( i%25 == 0 ):
        ...
        if ( action == 'NOTICE'):
            ...
            x['subject']='Himalaya Properties: Hiring @ Bowling Green'
            ...
            if ( attachment != ""):
                x['attachment']=attachment
            x['subject']='HP Tenant : Using Venmo or CashApp to pay rent'
            x['template_file']='HP Tenant Payment Message.html'
            noticeEmail(app, x)
        elif ( action == 'CARD'):
            x['template_file']='HP Tenant Message Card.html'
            x['subject']='HP: Happy Thanksgiving'
            if ( 'email' not in x or x['email'] == '' ):
                x['email']='pgp6758@gmail.com'
            noticeEmail(app, x)
        elif ( action == 'MESSAGE'):
            x['template_file']='HP Tenant Message.html'
            x['subject']='HP: Fall HVAC Filter change'
            if ( 'email' not in x or x['email'] == '' ):
                x['email']='pgp6758@gmail.com'
            noticeEmail(app, x)
            ...
            app.logger.debug("Not a valid action")
            
    app.logger.debug("processTenants.10 :Processed :[%s]%s", i, action)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"ht:m:a:c:g:f:i:",["attachment=","action=","company=","tenant_ids="])
    except getopt.GetoptError:
        print('pm-b-tenants.py -a <STATEMENTS | FINANCIALS | EMAIL | NOTICE> -t <tdate> -c <company name> -f <attachment> -i <tenant ids> -m <mode>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            ...
            tenants = arg
    print("Processing for [", action, "]date=", tdate , " company=", company, " group=", group)
    ...
    cfg = platform.system().lower()+"-"+mode
    print("OS:", os.name, " 2:", platform.system(), " config file:", hc_config, "cfg:", cfg)    
        
    CONFIG = utils.pm_conf.read_conf_from_file(hc_config, cfg)
    ...
    if ( company != "" or group != "" or tenants != "" ):
        processTenants(app, db,action, tdate, company, attachment, group, tenants)
    else:
        print('pm-b-tenants.py. ERR. Please provide company name, or a tenant info')
